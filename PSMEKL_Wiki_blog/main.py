...
def main(dataset, type):
    ...
    if args.dataset == 'wiki':
        temp = np.loadtxt('data/wiki/Wiki_edgelist.txt', dtype=np.long).T
        index = temp[0, :].argsort()
        edge_list = torch.tensor(temp[:, index], dtype=torch.long)
        labels = list(np.loadtxt('data/wiki/Wiki_category.txt')[:, -1])
    ...
    if args.dataset == 'blogcatalog':
        ...
        temp = scipy.io.loadmat('data/blogcatalog/blogcatalog.mat')
        network = temp['network'].tocoo()
        labels = temp['group'].toarray().argmax(axis=1).tolist()
        indices = np.vstack((network.row, network.col))
        edge_list = torch.LongTensor(indices)
