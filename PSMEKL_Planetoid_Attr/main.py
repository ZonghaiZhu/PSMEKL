...
num_subsets_dict = {'cora': 4, 'citeseer': 4, 'pubmed': 5, 'photo': 3, 'computers': 5}
...
parser.add_argument('--emb_type', type=str, default='PSMEKL',
                    help='Original, PSMEKL')
parser.add_argument('--combined', type=int, default=1)
parser.add_argument('--emb_dim', type=int, default=512, help='hidden size for node feature')
...
parser.add_argument("--run_times", type=int, default=20, help="seed for initializing training.")
...
def main():
    ...
    if not os.path.exists('log'):
        os.makedirs('log')
    log_dir = 'log/' + args.dataset + '_' + args.gnn + '_' + args.emb_type + '_' + str(args.combined) + '_' + time.strftime("%Y-%m-%d_%H-%M-%S")
    os.makedirs(log_dir)
    ...
    if args.combined:
        emb_feats = torch.cat((dataset[0].x.to('cuda'), emb_feats), dim=1)
    ...
    temp_results = np.array(temp, dtype=np.float32)
