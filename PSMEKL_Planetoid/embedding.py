...
def node_emb(data, emb_dim, emb_layer, emb_type, num_subsets):
    ...
    if emb_type == 'eigen' or emb_type == 'eigen_norm':
        ...
        try:
            if emb_type == "eigen":
                v = np.load(os.path.join(saved_path, data.name + '_' + str(emb_layer)) + '.npy')
            else:
                v = np.load(os.path.join(saved_path, data.name + '_' + 'degree_normalized' + '_' + str(emb_layer) + '.npy'))
            print(v.shape)
        except:
            # adj_matrix = nx.to_numpy_array(G) # for citeseer, this return 3279x3279, but it should be 3327x3327
            ...
            if emb_type == "eigen":
                np.save(os.path.join(dir, data.name + '_' + str(emb_layer) + '.npy'), v[:2000]) # only save top 1000 eigenvectors
            else:
                np.save(os.path.join(saved_path, data.name + '_' + 'degree_normalized' + '_' + str(emb_layer) + '.npy'), v[:2000])
